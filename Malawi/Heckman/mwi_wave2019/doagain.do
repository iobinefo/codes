...
global mwi_GHS_W4_created_data  "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\mwi_wave2019"
...
use "${mwi_GHS_W4_raw_data}\ag_mod_a_19.dta", clear

keep y4_hhid ag_b101 
rename (ag_b101) (ag_rainy_19)

tab ag_rainy_19
save "${mwi_GHS_W4_created_data}\ag_rainy_19.dta", replace
...
destring plot_srtm, replace force
recast byte plot_srtm

destring plot_srtmslp, replace force
recast byte plot_srtmslp



collapse (max) plot_twi plot_srtm  plot_srtmslp, by (y4_hhid)

merge 1:m y4_hhid using  "${mwi_GHS_W4_raw_data}\householdgeovariables_y4.dta"

merge m:1 y4_hhid using "${mwi_GHS_W4_created_data}\ag_rainy_19.dta", gen(filte)
...
ren plot_srtm plot_slope
ren plot_srtmslp plot_elevation
...
tab1 plot_wetness plot_slope plot_elevation, missing
...
ren HHID y4_hhid
merge m:1 y4_hhid using  "${mwi_GHS_W4_created_data}\hhids.dta", gen(hhids)
...
keep y4_hhid plot_slope plot_elevation plot_wetness annual_precipitation annual_mean_temp dist_admarc_w
...
*ren hh_a02a ta
rename panelweight_2019 weight
...
* PLOT AREAS *
********************************************************************************
use "${mwi_GHS_W4_raw_data}\ag_mod_c_19.dta",clear  
// HS 2.3.23: RAINY SEASON crop data; data about PLOT ID, Garden ID (how many plots per HH? How many gardens and how many plots in that garden?) GPS conditions, area reporting info, etc.
merge m:1 y4_hhid using "${mwi_GHS_W4_created_data}\ag_rainy_19.dta", gen(filte)
...
save "${mwi_GHS_W4_created_data}\Malawi_wave4_completedatap_2019.dta", replace
...
use "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\mwi_wave2010\Malawi_wave1_completedata_2010p.dta", clear

append using "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\mwi_wave2013\Malawi_wave2_completedata_2013p.dta" 

append using "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\mwi_wave2016\Malawi_wave3_completedata_2016p.dta" 

append using "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\mwi_wave2019\Malawi_wave4_completedatap_2019.dta"
...
egen median_maiz = median(maize_price_mr)
replace maize_price_mr = median_maiz if maize_price_mr==.

replace ext_access = 0 if ext_access==.
replace attend_sch = 0 if attend_sch==.
replace soil_qty_rev2 = 3 if soil_qty_rev2==.
replace subsidy_dummy = 0 if subsidy_dummy==.
replace subsidy_qty_w = 0 if subsidy_qty_w==.


egen median_dis = median (dist_admarc_w)
replace dist_admarc_w = median_dis if dist_admarc_w==.
...
*save "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\Malawi_complete_datapn.dta", replace
save "C:\Users\obine\Music\Documents\Project\codes\Malawi\Heckman\Malawi_complete_datap.dta", replace
