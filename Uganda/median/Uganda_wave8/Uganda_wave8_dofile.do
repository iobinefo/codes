...
replace rice_price=CEB14c *20 if CEB01==110 & CEB03C==109
...
replace rice_price =  8335 if rice_price > 8335 & rice_price<.  //bottom 2%
...
replace rice_price = medianri_pr_district if rice_price ==. & num_ri_district >= 5

tab rice_price,missing



replace rice_price = medianri_pr_stratum if rice_price ==. & num_ri_stratum >= 17
...
tab CEB06,missing
replace CEB10 = 0 if CEB10<=0 | CEB10==.
tab CEB10,missing

gen net_seller = 1 if CEB10 > CEB06
tab net_seller,missing
replace net_seller=0 if net_seller==.
tab net_seller,missing

gen net_buyer = 1 if CEB10 < CEB06
...
*drop _merge
*sort hhid
*merge 1:1 hhid using "C:\Users\obine\Music\Documents\Project\codes\Uganda\Uganda_wave6\soil_quality_2018.dta"
*drop _merge
sort hhid
...
misstable summarize femhead  ext_acess attend_sch  informal_credit formal_credit  total_qty_w  real_tpricefert_cens_mrk  num_mem hh_headage real_hhvalue worker land_holding  real_maize_price_mr real_rice_price_mr net_seller net_buyer safety_net
...
tabstat total_qty_w real_tpricefert_cens_mrk real_maize_price_mr real_rice_price_mr  num_mem hh_headage real_hhvalue worker land_holding [aweight = weight], statistics( mean median sd min max ) columns(statistics)

*  pry_edu finish_pry finish_sec  mrk_dist_w
replace real_tpricefert_cens_mrk= 0 if real_tpricefert_cens_mrk==.
