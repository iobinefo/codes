...
global tza_GHS_W2_created_data  "C:\Users\obine\Music\Documents\Project\codes\without_median\tanzania_copy\winsor\tza_wave2010"
...
collapse (max) plot_slope plot_elevation plot_wetness , by (HHID)
...
gen rea_tpricefert_cens_mrk = tpricefert_cens_mrk  /0.5165365
gen rea_tpricefert_cens_mrk2 = tpricefert_cens_mrk_w  /0.5165365
gen real_tpricefert_cens_mrk = rea_tpricefert_cens_mrk
gen real_tpricefert_cens_mrk2 = rea_tpricefert_cens_mrk2
...
keep HHID region ea district rural dist_cens_w total_qty_w total_valuefert real_tpricefert_cens_mrk real_tpricefert_cens_mrk2
 ...
 */
 use "${tza_GHS_W2_raw_data}\HH_SEC_FILTERS.dta",clear 
...
merge m:1 HHID using "${tza_GHS_W2_created_data}\tanzania_wave2_completedata_2010.dta", gen(file)
...
egen median_pr_ea = median(real_maize_price_mr), by (ea)
egen median_maize = median(real_maize_price_mr)
replace real_maize_price_mr = median_pr_ea if real_maize_price_mr==. 
tab real_maize_price_mr,missing
replace real_maize_price_mr = median_maize if real_maize_price_mr==. 
tab real_maize_price_mr,missing

egen median_rice_ea = median(real_rice_price_mr), by (ea)
egen median_rice = median(real_rice_price_mr)
replace real_rice_price_mr = median_rice_ea if real_rice_price_mr==. 
tab real_rice_price_mr,missing
...
misstable summarize subsidy_dummy femhead formal_credit ext_acess attend_sch pry_edu finish_pry finish_sec safety_net net_seller net_buyer soil_qty_rev2 total_qty_w subsidy_qty_w dist_cens_w real_tpricefert_cens_mrk num_mem hh_headage_mrk worker real_maize_price_mr real_rice_price_mr real_hhvalue field_size_ha_w good fair 
proportion subsidy_dummy femhead formal_credit ext_acess attend_sch pry_edu finish_pry finish_sec safety_net net_seller net_buyer soil_qty_rev2
